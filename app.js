...
function renderInventoryFromState() {
    ...
    // 1. Remove ANY old standalone buttons hanging around (except our container)
    const oldBtns = card.querySelectorAll('button');
    oldBtns.forEach(b => {
        if (b.parentNode.id !== 'inv-actions' && !b.closest('table')) b.remove();
    });
    ...
    if (!items || items.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5">Sin insumos...</td></tr>';
        return;
    }
    ...
    items.forEach((item) => {
        ...
        tdName.innerHTML = `<div style="font-weight:bold">${item.name}</div><div style="color:#888; font-size:12px">${item.id}</div>`;

        const tdStock = document.createElement('td');
        tdStock.style.fontWeight = 'bold';
        tdStock.innerText = Number(item.current_stock).toFixed(2);
        ...
        if (Number(item.current_stock) <= Number(item.min_stock)) {
            ...
            tdStatus.innerHTML = '<span class="badge" style="background:green;">OK</span>';
        }
    ...
    const thead = document.querySelector('#view-inventory thead tr');
...
async function viewInventoryHistory(item) {
    ...
    let html = `
        ...
        <table style="width:100%; border-collapse:collapse; font-size:13px;">
            <thead style="background:#f5f5f5; border-bottom:1px solid #ddd; position:sticky; top:0;">
                <tr>
